...
void LCD_Init(void)
{
  ...
  */
  ...
  #endif
  #if (LCD_EBI_BL_ENABLE == 1)
  HT32F_DVB_GPxConfig(LCD_EBI_BL_GPIO_ID, LCD_EBI_BL_GPIO_PIN, LCD_EBI_BL_AFIO_MODE);
  #endif
  #if (LCD_EBI_RST_UNUSE == 0)
  ...
  /* Configure the direction of BL as output mode                                                           */
  #if (LCD_EBI_BL_ENABLE == 1)
  GPIO_DirectionConfig(GPIO_PORT[LCD_EBI_BL_GPIO_ID], LCD_EBI_BL_GPIO_PIN, GPIO_DIR_OUT);
  #endif
...
void LCD_Config(void)
{
  ...
  #if (LCD_EBI_BL_ENABLE == 1)
  GPIO_SetOutBits(GPIO_PORT[LCD_EBI_BL_GPIO_ID], LCD_EBI_BL_GPIO_PIN);
  #endif
}
...
/*********************************************************************************************************//**
  * @brief  Read value from the selected LCD register.
  * @param  LCD_Reg_Index: address of the selected register.
  * @retval EBI_LCD->EBI_LCD_RAM
  ***********************************************************************************************************/
...
void LCD_WriteRAM(u32 RGB_Set)
{
  ...
  #else
  EBI_LCD->EBI_LCD_RAM = (RGB_Set >> 8) & 0x00FF;
  EBI_LCD->EBI_LCD_RAM = RGB_Set & 0xFF;
  #endif
}
...
void LCD_Clear(u16 Color)
{
  ...
  while (i--)
  {
    ...
    #else
    EBI_LCD->EBI_LCD_RAM = (Color >> 8) & 0x00FF;
