...
u32 SPI_FLASH_Init(void)
{
  ...
  */
  ...
  FLASH_SPI_MOSI_CLK(CKCUClock) = 1;
  CKCU_PeripClockConfig(CKCUClock, ENABLE);
  GPIO_DriveConfig(GPIO_PORT[FLASH_SPI_SEL_GPIO_ID], FLASH_SPI_SEL_GPIO_PIN, GPIO_DV_8MA);
  GPIO_DriveConfig(GPIO_PORT[FLASH_SPI_SCK_GPIO_ID], FLASH_SPI_SCK_AFIO_PIN, GPIO_DV_8MA);
  GPIO_DriveConfig(GPIO_PORT[FLASH_SPI_MOSI_GPIO_ID], FLASH_SPI_MOSI_AFIO_PIN, GPIO_DV_8MA);
  #endif
...
void SPI_FLASH_PageWriteByDMA(u8* WriteBuffer, u32 WriteAddr, u32 NumOfWriteByte)
{
  ...
  */
  ...
  {
    uSPIRxDmaChNumber = PDMA_SPI0_RX;
    uSPITxDmaChNumber = PDMA_SPI0_TX;
    SPIRxDmaCh = (HT_PDMACH_TypeDef *)(HT_PDMA_BASE + PDMA_SPI0_RX * 6 * 4);
    SPITxDmaCh = (HT_PDMACH_TypeDef *)(HT_PDMA_BASE + PDMA_SPI0_TX * 6 * 4);
  }
  #ifdef HT_SPI1
  else if (FLASH_SPI == HT_SPI1)
  {
    uSPIRxDmaChNumber = PDMA_SPI1_RX;
    uSPITxDmaChNumber = PDMA_SPI1_TX;
    SPIRxDmaCh = (HT_PDMACH_TypeDef *)(HT_PDMA_BASE + PDMA_SPI1_RX * 6 * 4);
    SPITxDmaCh = (HT_PDMACH_TypeDef *)(HT_PDMA_BASE + PDMA_SPI1_TX * 6 * 4);
  }
  #endif
  #ifdef HT_QSPI
  else if (FLASH_SPI == HT_QSPI)
  {
    uSPIRxDmaChNumber = PDMA_QSPI_RX;
    uSPITxDmaChNumber = PDMA_QSPI_TX;
    SPIRxDmaCh = (HT_PDMACH_TypeDef *)(HT_PDMA_BASE + PDMA_QSPI_RX * 6 * 4);
    SPITxDmaCh = (HT_PDMACH_TypeDef *)(HT_PDMA_BASE + PDMA_QSPI_TX * 6 * 4);
  }
  #endif
...
#ifdef HT_QSPI
/*********************************************************************************************************//**
  * @brief  Writes block of data to the FLASH with a quad WRITE cycle.
  * @param  WriteBuffer : pointer to the buffer  containing the data to be written to the FLASH.
  * @param  WriteAddr : FLASH's internal address to write to.
  * @param  NumOfWriteByte : number of bytes to write to the FLASH.
  * @retval None
  ***********************************************************************************************************/
void QSPI_FLASH_QuadPageWrite(u8* WriteBuffer, u32 WriteAddr, u32 NumOfWriteByte)
{
  /* Enable the write access to the FLASH                                                                   */
  SPI_FLASH_WriteEnable();
  ...
  /* Send "Quad Page Program" instruction                                                                   */
  SPI_FLASH_SendByte(SPI_FLASH_CMD_4PP);

  /* Send 6 Address cycle                                                                                   */
  SPI_SetDataLength(FLASH_SPI, SPI_DATALENGTH_2);
  QSPI_QuadCmd(FLASH_SPI, ENABLE);
  QSPI_DirectionConfig(FLASH_SPI, SIO_DIR_OUT);
  /* Send high byte address of WriteAddr                                                                    */
  SPI_FLASH_SendByte((WriteAddr & 0xFF0000) >> 16);
  /* Send medium byte address of WriteAddr                                                                  */
  SPI_FLASH_SendByte((WriteAddr & 0xFF00) >> 8);
  ...
  /* Disable quad port and reconfigure data frame length                                                    */
  QSPI_QuadCmd(FLASH_SPI, DISABLE);
  SPI_SetDataLength(FLASH_SPI, SPI_DATALENGTH_8);
...
#if defined (HT_QSPI) && defined (HT_PDMA)
/*********************************************************************************************************//**
  * @brief  Writes block of data to the FLASH with a quad WRITE cycle by DMA.
  * @param  WriteBuffer : pointer to the buffer  containing the data to be written to the FLASH.
  * @param  WriteAddr : FLASH's internal address to write to.
  * @param  NumOfWriteByte : number of bytes to write to the FLASH.
  * @retval None
  ***********************************************************************************************************/
void QSPI_FLASH_QuadPageWriteByDMA(u8* WriteBuffer, u32 WriteAddr, u32 NumOfWriteByte)
{
  PDMACH_InitTypeDef PDMACH_InitStructure_TX;
  PDMACH_InitTypeDef PDMACH_InitStructure_RX;
  HT_PDMACH_TypeDef *SPITxDmaCh;
...
#ifdef HT_QSPI
/*********************************************************************************************************//**
  * @brief  Reads a block of data from the FLASH using Quad Output Mode.
  * @param  ReadBuffer : pointer to the buffer that receives the data read from the FLASH.
  * @param  ReadAddr : FLASH's internal address to read from.
  * @param  NumOfReadByte : number of half word (16-bit) to read from the FLASH.
  * @retval None
  ***********************************************************************************************************/
void QSPI_FLASH_BufferQuadRead(u16* ReadBuffer, u32 ReadAddr, u32 NumOfReadByte)
{
  /* Select the SPI FLASH                                                                                   */
  SPI_FLASH_SEL_ACTIVE();

  /* Send "Read from Memory " instruction                                                                   */
  SPI_FLASH_SendByte(SPI_FLASH_CMD_QREAD);
  /* Send high byte address of ReadAddr                                                                     */
  SPI_FLASH_SendByte((ReadAddr & 0xFF0000) >> 16);
  /* Send medium byte address of ReadAddr                                                                   */
  SPI_FLASH_SendByte((ReadAddr& 0xFF00) >> 8);
  ...
  /* Enable Quad Port                                                                                       */
  SPI_SetDataLength(FLASH_SPI, SPI_DATALENGTH_4);
  QSPI_QuadCmd(FLASH_SPI, ENABLE);
  QSPI_DirectionConfig(FLASH_SPI, SIO_DIR_IN);
...
#if defined (HT_QSPI) && defined (HT_PDMA)
/*********************************************************************************************************//**
  * @brief  Reads a block of data from the FLASH using Quad Output Mode by DMA.
  * @param  ReadBuffer : pointer to the buffer that receives the data read from the FLASH.
  * @param  ReadAddr : FLASH's internal address to read from.
  * @param  NumOfReadByte : number of half word (16-bit) to read from the FLASH.
  * @retval None
  ***********************************************************************************************************/
void QSPI_FLASH_BufferQuadReadByDMA(u16* ReadBuffer, u32 ReadAddr, u32 NumOfReadByte)
{
  PDMACH_InitTypeDef PDMACH_InitStructure_TX;
  PDMACH_InitTypeDef PDMACH_InitStructure_RX;
  HT_PDMACH_TypeDef *SPITxDmaCh;
  ...
  /* QSPI Tx PDMA channel configuration                                                                     */
  PDMACH_InitStructure_TX.PDMACH_SrcAddr = (u32) &spi_dummy_data;
  PDMACH_InitStructure_TX.PDMACH_DstAddr = (u32) &FLASH_SPI->DR;
  PDMACH_InitStructure_TX.PDMACH_BlkLen = 1;
  PDMACH_InitStructure_TX.PDMACH_DataSize = WIDTH_8BIT;
  ...
  /* QSPI Rx PDMA channel configuration                                                                     */
  PDMACH_InitStructure_RX.PDMACH_SrcAddr = (u32) &FLASH_SPI->DR;
  PDMACH_InitStructure_RX.PDMACH_DstAddr = (u32) &ReadBuffer[0];
  PDMACH_InitStructure_RX.PDMACH_BlkLen = 1;
  PDMACH_InitStructure_RX.PDMACH_DataSize = WIDTH_16BIT;
...
/*********************************************************************************************************//**
  ...
  * @retval None
  ***********************************************************************************************************/
u8 SPI_FLASH_WriteStatus2(u8 Status_Value, u8 Configuration_Value)
{
  SPI_FLASH_WriteEnable();
  ...
  /* Send "Write Status Register" instruction                                                               */
  SPI_FLASH_SendByte(SPI_FLASH_CMD_WRSR);
  /* Writes to the Status / Configuration Register                                                          */
  SPI_FLASH_SendByte(Status_Value);
  /* Writes to the Status / Configuration Register                                                          */
  SPI_FLASH_SendByte(Configuration_Value);
