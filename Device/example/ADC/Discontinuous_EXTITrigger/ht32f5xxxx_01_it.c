/*********************************************************************************************************//**
 * @file    ADC/Discontinuous_EXTITrigger/ht32f5xxxx_01_it.c
 * @version $Rev:: 4680         $
...
void HTCFG_ADC_IRQHandler(void)
{
  ...
  if (ADC_GetIntStatus(HTCFG_ADC_PORT, ADC_FLAG_CYCLE_EOC))
  {
    /* Get the last sub group ADC result                                                                    */
    ADC_ClearIntPendingBit(HTCFG_ADC_PORT, ADC_FLAG_CYCLE_EOC | ADC_FLAG_SUB_GROUP_EOC);
  ...
  else
  {
    ...
    ADC_ClearIntPendingBit(HTCFG_ADC_PORT, ADC_FLAG_SUB_GROUP_EOC);
