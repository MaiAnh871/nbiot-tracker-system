...
void I2S_Configuration(void)
{
  ...
  */
  ...
  I2S_InitStructure.I2S_MclkInv = DISABLE;
...
void I2CM_Init(void)
{
  ...
  */
  ...
  I2C_Init(HTCFG_I2CM_ID, &I2C_InitStructure);

  I2C_SetTimeOutPrescaler(HTCFG_I2CM_ID, I2C_PRESCALER_128);
  I2C_SetTimeOutValue(HTCFG_I2CM_ID, ((SystemCoreClock/128) * 8 * 10) / I2CM_SPEED); /* Must within 10 Bytes Time    */

  /* Enable I2C                                                                                             */
  I2C_Cmd(HTCFG_I2CM_ID, ENABLE);
  I2C_TimeOutCmd(HTCFG_I2CM_ID, ENABLE);

  /* Enable I2C interrupts                                                                                  */
  I2C_IntConfig(HTCFG_I2CM_ID, I2C_INT_STA | I2C_INT_ADRS | I2C_INT_RXDNE | I2C_INT_TXDE |\
...
u32 I2CM_GetTransferStatus(void)
{
  ...
  if (pTransfer->Status != I2CM_BUSY)
  {
    ...
    while (I2C_GetFlagStatus(HTCFG_I2CM_ID, I2C_FLAG_BUSBUSY))
    {
      I2CM_DelayXuS(100);
      timeout--;
      if (timeout == 0)
