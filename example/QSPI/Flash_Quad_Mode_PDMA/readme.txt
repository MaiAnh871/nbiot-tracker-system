/**
 @page QSPI_Flash_Quad_Mode_PDMA

 @verbatim
 * @file    QSPI/Flash_Quad_Mode_PDMA/readme.txt
 * @version V2.00
 * @date    2020-07-27
 * @brief   Description of the QSPI_Flash PDMA example.
 @endverbatim
...
PDMA channel n1 is configured to transmit data from HT_QSPI->DR to rxBuffer for QSPI receiver.
PDMA channel n2 is configured to transmit data from WriteBuffer to HT_QSPI->DR for QSPI transmitter.
...
Test Steps
  ...
  - Program the data of "gWrite_Buffer[]" into the address "FLASH_TEST_ADDR" of the flash by qual mode
    with PDMA.
  - Read the Flash data to the variable, "gRead_Buffer[]" by qual read function with PDMA.
  ...
  - Do the flash blank check (by qual read function with PDMA), turn on the LED3 if the blank check is passed.

@par Directory Contents:

- QSPI/Flash_Quad_Mode_PDMA/main.c                          Main program
- QSPI/Flash_Quad_Mode_PDMA/ht32fxxxx_it.c                  Interrupt handlers
