/*********************************************************************************************************//**
 * @file    USART/RS485_NMM_Slave/ht32f5xxxx_01_it.c
 * @version $Rev:: 5435         $
...
void HTCFG_UART_IRQHandler(void)
{
  ...
  if (USART_GetFlagStatus(HTCFG_UART_PORT, USART_FLAG_RSADD))
  {
    extern vu32 guSlaveState;
    u8 uSlaveAddr;

    USART_ClearFlag(HTCFG_UART_PORT, USART_FLAG_RSADD);
    uSlaveAddr = USART_ReceiveData(HTCFG_UART_PORT);

    if (uSlaveAddr == HTCFG_RS485_SLAVE_ADDR)
    {
      USART_RxCmd(HTCFG_UART_PORT, ENABLE);
      guSlaveState = RS485_STATE_ADDRESSED;
    }
    else
    {
      #if 1
      /* !!! NOTICE !!!
         A data packet (Parity = 0) is required after received a unmatch address packet to receive the
         next address packet.
      */
      USART_RxCmd(HTCFG_UART_PORT, DISABLE);
      #else
      /* !!! NOTICE !!!
         Must reset USART IP if no data packet (Parity = 0) after received a unmatch address packet.
      */
      extern void UxART_Configuration(void);
      USART_DeInit(HTCFG_UART_PORT);
      UxART_Configuration();
      #endif

      guSlaveState = RS485_STATE_IDLE;
    }
  }
