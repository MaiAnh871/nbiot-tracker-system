/*********************************************************************************************************//**
 * @file    I2S/CodecLoopback_PDMA/ht32f5xxxx_01_it.c
 * @version $Rev:: 4823         $
 ...
 * @retval  None
 ************************************************************************************************************/
void HTCFG_I2C_IRQHandler(void)
{
  extern I2CM_TransferType I2CM_Transfer;
  u32 state = HTCFG_I2C_PORT->SR;
  I2CM_TransferType* pTransfer = &I2CM_Transfer;

  HTCFG_I2C_PORT->SR = state;
  ...
  if (state & I2C_FLAG_RXNACK)
  {
    if (pTransfer->RetryDownCounter)
    {
      ...
      I2C_TargetAddressConfig(HTCFG_I2C_PORT, pTransfer->DevAddr, I2C_MASTER_WRITE);
      return;
    }
    else
    {
  ...
  if (pTransfer->Buffer)
  {
    switch (state)
    {
      ...
      case I2C_MASTER_TRANSMITTER_MODE:
          ...
          {
            HTCFG_I2C_PORT->DR = pTransfer->RegAddr >> 8;
            pTransfer->IsRegAddrSent = FALSE;
          }
          else
          {
            HTCFG_I2C_PORT->DR = pTransfer->RegAddr;
      ...
      case I2C_MASTER_TX_EMPTY:
          if (pTransfer->IsRegAddrSent)
          {
            if (pTransfer->Direction == I2CM_DIRECTION_OUT)
            {
              ...
              {
                HTCFG_I2C_PORT->DR = pTransfer->Buffer[pTransfer->Counter++];
              }
              else
              {
                pTransfer->Status = I2CM_OK;
                /* Send I2C STOP condition                                                                  */
                I2C_GenerateSTOP(HTCFG_I2C_PORT);
            ...
            else
            {
              ...
              I2C_TargetAddressConfig(HTCFG_I2C_PORT, pTransfer->DevAddr, I2C_MASTER_READ);
            }
          }
          else
          {
            HTCFG_I2C_PORT->DR = pTransfer->RegAddr & 0xFF;
            pTransfer->IsRegAddrSent = TRUE;
          }
        break;
      ...
      case I2C_MASTER_RECEIVER_MODE:
          ...
          {
            I2C_AckCmd(HTCFG_I2C_PORT, ENABLE);
          }
        break;

      case I2C_MASTER_RX_NOT_EMPTY:
          /* Receive data sent from I2C                                                                     */
          pTransfer->Buffer[pTransfer->Counter++] = HTCFG_I2C_PORT->DR;
          if (pTransfer->Counter == pTransfer->Length - 1)
          {
            /* Disable I2CM ACK                                                                             */
            I2C_AckCmd(HTCFG_I2C_PORT, DISABLE);
          }
          if (pTransfer->Counter == pTransfer->Length)
          {
            pTransfer->Status = I2CM_OK;
